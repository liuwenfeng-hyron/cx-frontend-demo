...
export class AssetViewerComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.filteredAssets$ = this.fetch$
      .pipe(
        switchMap(() => {
          const assets$ = this.assetService.requestAssets({"limit" :  10000000});
  ...
  // Added By Nri 2025.3.6 Start
  onUpdate(assetInfo: Asset) {    
    const asset: AssetInput = {
      "@id": assetInfo.id,
      properties: assetInfo.properties,
      dataAddress: {
        "type": assetInfo.dataAddress.optionalValue('edc','type') || '',
        "name": assetInfo.dataAddress.optionalValue('edc','name'),
    ...
    dialogRef.afterClosed().pipe(first()).subscribe((result: { assetInput?: AssetInput }) => {
      const newAsset = result?.assetInput;
      if (newAsset) {
        this.assetService.updateAsset(newAsset).subscribe({
          next: ()=> this.fetch$.next(null),
          error: err => this.showError(err, "This asset cannot be updated"),
